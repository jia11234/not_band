...
export default function IdFind() {
  ...
  //회원가입에선 아이디를 입력하지만 여기선 이미 아이디가 존재하기때문에 존재하지 않는 임의의 아이디를 넣었음
...
//유효성 검사
  ...
  const signUpSubmit = (e) => {
    ...
    if (data.email === "") {
      ...
      newMessage.phoneNumber = "휴대폰 번호를 입력해주세요";
    } else {
      if (data.phoneNumber && data.email) {
        const foundUser = user.find(
          (user) =>
            user.memPhone === data.phoneNumber && user.memEmail === data.email,
        );

        if (foundUser) {
          navigate("/not_band/id-findOk", {
            state: {
