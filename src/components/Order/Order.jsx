...
import { getCartRequest, getUserRequest, allProducts, addToOrder, pointPlus, pointMinus } from '../../apis';
...
export default function Order() {
    ...
    const {
        prdNo, addPrd1, addPrd2, ctQty,
        add1Qty, add2Qty, colorOpt, cartChecked,
    } = state || {};
    ...
    const fetchUser = async (memId) => {
        try {
            ...
            console.log("나오겟지이건ㅍㅍ", response);
            setUser(response);
            setZipCode(response.memZipcode);
            setAddress(response.memAddress);
            setDetailAddress(response.memDetialAddress);
    ...
    const calculateItemPrice = (item) => {
        const productPrice = prices[item.prdNo]?.originalPrice || Number(getProductName5(item.prdNo)) * (item.ctQty || 1);
        const addPrd1Price = item.addPrd1 ? Number(item.add1Qty * getProductPrice1(item.addPrd1)) : 0;
        const addPrd2Price = item.addPrd2 ? Number(item.add2Qty * getProductPrice2(item.addPrd2)) : 0;
    ...
    const handleAddToOrder = async () => {
        ...
        try {
            ...
            console.log("결과과고", memId, usedPoint, pluspoint);
        } catch (error) {
    ...
    const handleKakaoPay = async (orderName, cartItems, totalAmount) => {
        ...
        try {
            const ordNo = await addToOrder(cartItem);
            const res = await axios.get("http://localhost:8080/api/v1/not_band/pay/ready", {
                params: {
                    userId: orderName,
            ...
            <div className='order_group'>
                ...
                {user && (
                    <div className="order_information">
                        ...
                        <div className="orderer_address">
                            <p>주소</p>
                            <input type="text" name="zipCode" value={data.zipCode || zipCode} onChange={dataChange} placeholder="우편 번호" readOnly />
                            <button onClick={handleAddressSearch}>우편번호 찾기</button>
                            <br />
                            <input type="text" name="address" value={data.address || address} onChange={dataChange} placeholder="주소" readOnly />
                            <br />
                            <input type="text" name="detailAddress" value={detailAddress} onChange={(e) => setDetailAddress(e.target.value)} placeholder="상세 주소" />
                        </div>
                        <div className="orderer_memo">
                            <div><input type="checkbox" onChange={selectedMemo} /> <p>배송 요청사항 개별 입력</p></div>
                            {memo ? (
                                <input type="text" value={deliveryRequest} placeholder='배송 요청사항을 입력해주세요' onChange={(e) => setDeliveryRequest(e.target.value)} />
                            ) : (
                                <select value={deliveryRequest} onChange={(e) => setDeliveryRequest(e.target.value)}>
                    ...
                    {cartItems.map((item, index) => (
                        <div className="order_list_group" key={index}>
                            {item.cartChecked == true && (
                                <table>
                                    <tr key={index} className={`order_list_tr ${index === cartItems.length - 1 ? 'last-item' : ''}`}>
                                        <td className='order_list_product'>
                                            <div className='order_list_product_group'>
                                                ...
                                                />
                                                <div>
                                                    <p style={{ marginBottom: item.colorOpt ? "2px" : "16px" }}>{getProductName(item.prdNo)} / {item.ctQty}개</p>
                                                    {item.colorOpt && (
                                                        <p className='order_color_option'>옵션: {item.colorOpt}</p>
                                                    )}
                                                    <div>
                                                        {isMobile ? (
                                                            <p>
                                                                {prices[item.prdNo]?.originalPrice.toLocaleString() || Number(getProductName5(item.prdNo)).toLocaleString()} 원&nbsp;
                                                                <br />
                                                                {getProductName6(item.prdNo) > 0 && (
                                                                    <span>
                                                                        {prices[item.prdNo]?.discountedPrice.toLocaleString() || Number(getProductName6(item.prdNo)).toLocaleString()} 원&nbsp;
                                                                    </span>
                                                                )}
                                                            </p>
                                                        ) : (
                                            ...
                                            {(item.addPrd1 || item.addPrd2) &&
                                                <div className='order_plus_product_group'>
                                                    <div className="order_plus_product">
                                                        <div>추가상품</div>
                                                        <div>
                                                            <p>{item.addPrd1 && `${getProductName2(item.addPrd1)} / ${item.add1Qty}개 (+${Number(item.add1Qty * (getProductPrice1(item.addPrd1))).toLocaleString()})`}</p>
                                                            <p>{item.addPrd2 && `${getProductName3(item.addPrd2)} / ${item.add2Qty}개 (+${Number(item.add2Qty * (getProductPrice2(item.addPrd2))).toLocaleString()})`}</p>
                                                        </div>
                                                    </div>
                                                </div>
                                            }
                ...
                {user && (
                    <div className="order_point" >
                        <p>보유 {user.memPoint.toLocaleString()}원</p>
                        <input type="text"
                            placeholder="사용하실 포인트를 입력해 주세요" value={`${usedPoint.toLocaleString()} 원`}
                            onChange={handleChange} />
                        <button onClick={() => handleUseAllPoints(user.memPoint)}>전액 사용</button>
                    </div>
                )}
                ...
                <div className="order_payment_group">
                    <div className="order_payment">
                        <div>
                            <input type="radio" name="payment" id="credit_card" onChange={() => setPaymentMethod("creditCard")}
                        ...
                        <div>
                            <input type="radio" name="payment" id="kakao" onChange={() => setPaymentMethod("kakao")}
                        ...
                        <div>
                            <input type="radio" name="payment" id="deposit" onChange={() => setPaymentMethod("deposit")}
                    ...
                    {paymentMethod === "creditCard" && (
                        <div className="order_credit_group">
                            <p>신용카드 결제 시 화면 아래 ‘결제하기’버튼을 클릭하시면 신용카드 결제 창이 나타납니다.<br />
                                신용카드 결제 창을 통해 입력되는 고객님의 카드 정보는 안전하게 암호화되어 전송되며,<br />
                                승인 처리 후 카드 정보는 승인 성공/실패 여부에 상관없이 자동으로 폐기되므로, 안전합니다.</p>
                        </div>
                    )}
                    {paymentMethod === "deposit" && (
                        <div className="order_deposit_group">
                            <select>
                                <option value="">입금 계좌번호 선택해주세요 *반드시 주문자 성함으로 입금해주세요*</option>
                                <option value="">농협중앙회 355-0876-6738-15 (예금주:(주)낫밴드)</option>
                                <option value="">국민은행 652001-01-472051 (예금주:(주)낫밴드)</option>
                            </select>
                        </div>
                    )}
                </div>
                ...
                <div className="order_price_group">
                    <div>
                        ...
                        <div className="order_price">
                            <p>사용 포인트</p>
                            <p>{usedPoint.toLocaleString()}포인트</p>
                        </div>
                        <div className="order_price">
                            <p>포인트 적립 2%</p>
                            <p>{(Number(selectedTotalPrice) * 0.02).toLocaleString()}포인트</p>
                        </div>
                        <div className="order_price_total">
                            <p>총 결제 금액</p>
                            <p>{(Number(selectedTotalPrice) + Number(usedPoint) + Number(deliveryPrice)).toLocaleString()
                            } 원</p>
                        </div>
                    </div>
                    <button className='btnRed' onClick={handleAddToOrder}>구매하기</button>
                </div>
            </div>
        </div>
    );
